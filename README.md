...
2. @RefreshScope 不能用在 @Scheduled、Listener、Timer等类上，会有问题。
...
1. 更新@Value值的方案：启动时扫描bean，如果有@Value，就记下来。在配置发生变化时，通过反射去修改@Value标注的字段。
2. 发布事件的方案：主要用于Apollo，监听Apollo专有的配置更新事件，发布为EnvironmentChangeEvent
